...
Logger.prototype.writable = function (stream) {
  this._streams.writable.push(stream);
  return this;
};

Logger.prototype.readable = function (stream) {
  var self = this;
  this._streams.readable.push(stream);
  
  stream.on('readable', function () {
    var event = this.read();
    if (event) self.logEvent(event);
  });
...
Logger.prototype.logEvent = function (event) {
  ...
  this._streams.writable.forEach(function (stream) {
    stream.write(event);
  });
  return this;
};
