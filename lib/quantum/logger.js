...
function Logger (namespace, opts) {
  ...
  this.useLevels = opts.levels || 'syslog';
  this.useTokens = opts.tokens || 'basic';
...
Logger.prototype.clone = function (ns) {
  ...
  var logger = new Logger(ns, {
      levels: this.useLevels
    , tokens: this.useTokens
  });

  logger.tokenQueue = this.tokenQueue.slice(0);
  logger.filterQueue = this.filterQueue.slice(0);
  logger.writeQueue = this.writeQueue.slice(0);
  logger.readQueue = this.readQueue.slice(0);
  return logger;
};
...
Logger.prototype.levels = function (spec) {
  if (!this.initialized) this.useLevels = spec;
  return this;
};
...
Object.defineProperty(Logger.prototype, 'tokens',
  { get: function () {
      ...
      this.tokenQueue.forEach(function (spec) {
      ...
      if (~[ 'process', 'memory' ].indexOf(this.useTokens)) {
        res.pid = process.pid;
...
Logger.prototype.start = function (cb) {
  ...
  function resolved (i) {
    return function parseSpec (spec) {
      if (spec.type == 'write')
        self.writeQueue.push(spec);
      else if (spec.type == 'read')
        self.readQueue.push(spec);
      iterate(i);
    }
  }
...
Logger.prototype.filter = function (condition) {
  if (condition) this.filterQueue.push(condition);
  return this;
};
...
function evaluateConditions (event) {
  ...
  this.filterQueue.forEach(function (condition) {
