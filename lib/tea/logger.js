...
Object.defineProperty(Logger.prototype, 'tokens',
  { get: function () {
      ...
      if (~[ 'process', 'memory' ].indexOf(this.useTokens)) {
        res.gid = process.getgid();
        res.uid = process.getuid();
        res.pid = process.pid;
      }
...
Logger.prototype.start = function (cb) {
  ...
  function resolved (i) {
    return function parseSpec (spec) {
      if (spec.type == 'write') self.writeQueue.push(spec);
      else if (spec.type == 'read') self.readQueue.push(spec);
      iterate(i);
    }
  }
